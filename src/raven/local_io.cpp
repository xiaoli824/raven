...
void teleopIntoDS1(struct u_struct *us_t)
{
    ...
    for (i=0;i<NUM_MECH;i++)
    {
        ...
        const int graspmax = (M_PI/2 * 1000); //pi/2
        const int graspmin = (-30.0 * 1000.0 DEG2RAD); //-30
		data1.rd[i].grasp -= 2*us_t->grasp[armidx]; //changed from -= at ICRA15
...
int init_ravenstate_publishing(ros::NodeHandle &n){
    ...
    pub_joint_sim = n.advertise<std_msgs::Float32MultiArray>("joint_states_sim", 1);
...
//        return;
    ...
    //float joint_temp[2][8];
    //for (int i = 0;i<8;i++)
    //{
	//for(int j=0;j<2;j++)
	//{
	  // joint_temp[i][j] = dev->mech[j].joint[i].jpos RAD2DEG;
	//}
    //}
    //std::cout<<joint_temp[1][1]<<std::endl;
...
//    msg_ravenstate.f_secs = d.toSec();
    ...
    for(int i=0;i<3;i++)
    {
	msg.data.push_back(dev->mech[1].joint[i].jpos_d);
    }

    for(int i=0;i<3;i++)
    {
	msg.data.push_back(dev->mech[0].joint[i].jpos_d);
    }

    //msg[0].data= dev->mech[1].joint[1].jpos;
    pub_joint_sim.publish(msg);
}
